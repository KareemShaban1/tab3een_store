...
class SuperadminSettingsController extends BaseController
{
    ...
    public function editPopupPost(){
        $post_popup = DB::table('setting')->select('popup_post','popup_time','popup_disable')->first();
        return view('superadmin::popup-post.edit',compact('post_popup'));
    }
    public function updatePopupPost(Request $request){

        DB::table('setting')->where('id',1)->update([
            'popup_post' =>$request->popup_post,
            'popup_time'=>$request->input('popup_time'),
            'popup_disable'=>$request->input('popup_disable')]);
       return redirect()
       ->action('\Modules\Superadmin\Http\Controllers\SuperadminSettingsController@editPopupPost');
    }
}
